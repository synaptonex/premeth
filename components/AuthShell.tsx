...
export default function AuthShell({ title, subtitle, children, footer }: Props) {
  ...
  return (
    <div ref={root} className="min-h-screen grid lg:grid-cols-2">
      {/* Left: visual */}
      <div className="relative hidden lg:flex flex-col justify-between p-10 bg-ink-950 border-r border-ink-800 overflow-hidden">
        <div
          aria-hidden
        ...
        <div className="relative max-w-sm">
          <blockquote className="font-display text-2xl text-paper leading-snug italic">
          ...
          <div className="mt-4 text-sm text-ink-400">— The Premeth team</div>
        </div>

        <div className="relative text-xs text-ink-500">
          © {new Date().getFullYear()} Premeth · Educational use only.
        </div>
      </div>
      ...
      <div className="flex items-center justify-center p-6 sm:p-10">
        <div className="w-full max-w-sm">
          ...
          <h1 className="auth-title font-display text-3xl md:text-4xl text-paper tracking-tight">
            {title}
          </h1>
          {subtitle && <p className="auth-sub mt-2 text-ink-400 text-sm">{subtitle}</p>}

          <div className="auth-form mt-7 space-y-4">{children}</div>

          {footer && <div className="auth-footer mt-6 text-sm text-ink-400">{footer}</div>}
        </div>
      </div>
    </div>
  );
