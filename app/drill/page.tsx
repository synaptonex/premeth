...
type Status = 'gate' | 'building' | 'ready' | 'finished' | 'empty';
  ...
  if (status === 'empty') {
    return (
      <>
        <Navbar />
        <main className="mx-auto max-w-lg px-5 py-20 text-center">
          <h1 className="font-display text-3xl text-paper mb-2">Drill couldn't be built</h1>
          <p className="text-ink-400 mb-6">
      ...
      <>
        ...
        <main className="mx-auto max-w-xl px-5 py-16">
          <div className="text-center mb-8">
            ...
            <h1 className="font-display text-4xl text-paper">Drill complete.</h1>
    ...
    <>
      ...
      <main className="mx-auto max-w-3xl px-5 py-8">
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            <span className="text-xs uppercase tracking-widest text-meth">Daily Drill</span>
            <h1 className="font-display text-2xl text-paper">
              Q{qIndex + 1} <span className="text-ink-500">/ {questions.length}</span>
            </h1>
          </div>
          <div className="text-right">
            <div className="text-xs text-ink-500">Topic</div>
            <div className="text-sm text-ink-200 truncate max-w-[12rem]">{q.topic}</div>
          </div>
        </div>

        {/* Progress bar */}
        <div className="h-1 rounded-full bg-ink-900 mb-6 overflow-hidden">
          <div
            className="h-full bg-meth transition-all duration-200"
        ...
        <div className="rounded-xl border border-ink-800 bg-ink-900/40 p-6 mb-4">
            ...
            {q.question.options.map((opt, i) => {
              ...
              return (
                ...
                >
                  ...
                  {reveal && isCorrect && <Check className="h-4 w-4 text-meth shrink-0" />}
          ...
          {showExplain && (q.question.explanation || q.question.options[correctIndex]?.explanation) && (
            <div className="mt-5 rounded-md border border-ink-800 bg-ink-950/60 p-4">
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="text-sm text-ink-500">
            Score so far: <span className="text-paper">{score}</span> / {qIndex + 1}
          </div>
          {!isSubmitted ? (
            <button
              onClick={submitAnswer}
              disabled={selected === null}
              className="press inline-flex items-center gap-2 rounded-md bg-meth text-ink-950 px-5 py-2.5 font-medium hover:bg-meth-300 tx-color disabled:opacity-50"
            >
              Submit
            </button>
          ) : (
