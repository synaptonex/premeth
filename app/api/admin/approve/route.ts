...
export async function POST(req: Request) {
  ...
  // All writes below go through the service-role client. The admin identity
  // is already verified above. Using the service-role client means a money
  // activation never depends on RLS evaluating correctly under the admin's
  // session, where a silent denial would leave a paid buyer with nothing.
  ...
  if (code) {
    await admin.from('redemption_codes').insert({
      code,
