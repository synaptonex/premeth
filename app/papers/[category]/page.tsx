  ...
  return (
    <>
      ...
      <main ref={root} className="mx-auto max-w-5xl px-5 py-12">
        <Link
          href="/exams"
          className="inline-flex items-center gap-1.5 text-sm text-coal-600 hover:text-accent tx-color mb-6"
        >
        ...
        <div className="mb-8">
          ...
          <p className="text-coal-600 mt-2 max-w-2xl">{info.description}</p>
          <div className="mt-4 text-sm text-coal-500">
            <span className="text-coal-900">{index.papers.length}</span> papers ·{' '}
            <span className="text-coal-900">
              {index.papers
              ...
              <button
                ...
                className={`press text-xs px-3 py-2 rounded-md border tx-color whitespace-nowrap ${
                  subjectFilter === null
                    ...
                    : 'border-coal-rule text-coal-700 hover:border-coal-400'
                }`}
              >
                All subjects
              </button>
              ...
              >
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="font-medium text-coal-900 group-hover:text-accent tx-color truncate">
                    ...
                    {p.year && (
                      <span className="text-xs text-coal-500">{p.year}</span>
                    )}
                  </div>
                  {p.topics?.length > 0 && (
                    <p className="text-xs text-coal-500 mt-1 truncate">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <span className="text-xs text-coal-600">
                    <span className="text-coal-900 font-medium">
                      {p.questionCount}
                    </span>{' '}
                    Qs
                  </span>
                  <ChevronRight className="h-4 w-4 text-coal-500 group-hover:text-accent group-hover:translate-x-1 transition-transform duration-200 ease-out-soft" />
                </div>
              </Link>
            </li>
          ))}
