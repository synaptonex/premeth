...
export default function MockExamPage() {
  ...
  if (sub.loading) {
    return (
      <><Navbar />
        <main className="mx-auto max-w-2xl px-5 py-20 text-center text-ink-400">Loading…</main>
      </>
    );
  }
      ...
      <><Navbar />
        <main className="mx-auto max-w-2xl px-5 py-12">
          <div className="text-center mb-10">
            <Clock className="h-10 w-10 text-meth mx-auto mb-3" />
            <h1 className="font-display text-4xl text-paper">Full timed mock exam.</h1>
            <p className="text-ink-400 mt-2">PMDC MDCAT format — 200 MCQs, 200 minutes.</p>
          </div>
          <div className="rounded-xl border border-ink-800 bg-ink-900/40 p-6 mb-8">
            <h3 className="font-medium text-paper mb-3">What to expect</h3>
            <ul className="space-y-2 text-sm text-ink-300">
              {Object.entries(SUBJECT_QUOTAS).map(([subj, n]) => (
                <li key={subj} className="flex justify-between">
                  <span>{subj}</span>
                  <span className="text-ink-500">{n} MCQs</span>
                </li>
              ))}
              <li className="flex justify-between border-t border-ink-800 pt-2 mt-2 font-medium">
                <span>Total</span>
                <span>{TOTAL_MCQS} MCQs · 200 min</span>
              </li>
            </ul>
          </div>
          <div className="rounded-md border border-amber-500/30 bg-amber-500/5 p-4 mb-6 flex items-start gap-3">
  ...
  if (status === 'building') {
    return (
      <><Navbar />
        <main className="mx-auto max-w-lg px-5 py-20 text-center">
          <Sparkles className="h-8 w-8 text-meth mx-auto mb-3 animate-pulse" />
          <p className="text-ink-300">Assembling your exam…</p>
          <p className="text-xs text-ink-500 mt-1">Sampling from 2,500+ past papers</p>
        </main>
      </>
    );
  }
  ...
  if (status === 'empty') {
    return (
      <><Navbar />
        <main className="mx-auto max-w-lg px-5 py-20 text-center">
          <h1 className="font-display text-3xl text-paper mb-2">Could not build exam</h1>
          <p className="text-ink-400">Not enough question data available right now. Try again later.</p>
        </main>
      </>
    );
  }
      ...
      <><Navbar />
        <main className="mx-auto max-w-xl px-5 py-12">
          <div className="text-center mb-8">
            ...
            <h1 className="font-display text-4xl text-paper">Exam complete.</h1>
            <p className="text-ink-400 mt-2">
              {score} / {questions.length} · <span className={pct >= 75 ? 'text-meth' : pct >= 50 ? 'text-amber-400' : 'text-crimson'}>{pct}%</span>
            </p>
          </div>
          <div className="rounded-xl border border-ink-800 bg-ink-900/40 p-5 mb-8">
            <h3 className="font-medium text-paper mb-3">By subject</h3>
                ...
                return (
                  <li key={subj}>
                    <div className="flex justify-between text-sm mb-1">
                      <span className="text-ink-200">{subj}</span>
                      <span className="text-ink-400">{b.correct} / {b.total} · {p}%</span>
                    </div>
                    <div className="h-1.5 rounded-full bg-ink-900 overflow-hidden">
                      <div
                        className={`h-full ${
                          p >= 75 ? 'bg-meth' : p >= 50 ? 'bg-amber-400' : 'bg-crimson'
                        }`}
  ...
  return (
    <>
      ...
      <main className="mx-auto max-w-6xl px-5 py-6">
        ...
        <div className="sticky top-14 z-20 -mx-5 px-5 py-3 mb-5 border-b border-ink-800 bg-ink-950/80 backdrop-blur-md">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <Clock className={`h-5 w-5 ${timeLeft < 600 ? 'text-crimson' : 'text-meth'}`} />
              <span className={`font-display text-2xl ${timeLeft < 600 ? 'text-crimson' : 'text-paper'}`}>
                {fmtMMSS(timeLeft)}
              </span>
            </div>
            <div className="text-sm text-ink-400">
              <span className="text-paper">{answeredCount}</span> / {questions.length} answered
            </div>
            <button
              onClick={() => setShowSubmitConfirm(true)}
              className="press text-sm rounded-md bg-meth text-ink-950 px-4 py-1.5 font-medium hover:bg-meth-300 tx-color"
            >
              Submit
            </button>
          </div>
        ...
        <div className="grid lg:grid-cols-[1fr,260px] gap-6">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <div>
                <span className="text-xs uppercase tracking-widest text-meth">{q.subject}</span>
                <h2 className="font-display text-xl text-paper">Q{qIndex + 1}</h2>
              </div>
              <button
                onClick={toggleFlag}
                className={`press inline-flex items-center gap-1.5 rounded-md border px-3 py-1.5 text-sm tx-color ${
                  flagged[qIndex] ? 'border-amber-400 text-amber-400' : 'border-ink-800 hover:border-ink-700'
                }`}
              >
            ...
            <div className="rounded-xl border border-ink-800 bg-ink-900/40 p-6 mb-4">
              <p className="text-paper leading-relaxed mb-5">{q.question.text}</p>
            ...
            <div className="flex items-center justify-between">
              <button
                ...
                disabled={qIndex === 0}
                className="press inline-flex items-center gap-1.5 rounded-md border border-ink-700 px-3 py-2 text-sm hover:border-ink-500 disabled:opacity-40 disabled:cursor-not-allowed tx-color"
              >
              ...
              <button
                ...
                className="press inline-flex items-center gap-1.5 rounded-md bg-meth text-ink-950 px-4 py-2 text-sm font-medium hover:bg-meth-300 disabled:opacity-40 disabled:cursor-not-allowed tx-color"
              >
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-32 rounded-xl border border-ink-800 bg-ink-900/40 p-3">
              <div className="text-xs uppercase tracking-wider text-ink-500 mb-2 px-1">Questions</div>
                  ...
                  return (
                    <button
                      ...
                      className={`press text-xs rounded h-7 w-full font-mono tx-color ${
                        isCurr
                          ? 'bg-meth text-ink-950'
                          : isAns
                          ? 'bg-meth/15 text-meth border border-meth/30'
                          : 'border border-ink-800 text-ink-400 hover:border-ink-600'
                      } ${isFlag ? 'ring-1 ring-amber-400' : ''}`}
              ...
              <div className="mt-3 pt-3 border-t border-ink-800 space-y-1 text-xs text-ink-400">
                <Legend color="bg-meth" label="Current" />
                <Legend color="bg-meth/15 border border-meth/30" label="Answered" />
                <Legend color="ring-1 ring-amber-400" label="Flagged" />
              </div>
            </div>
          </aside>
        </div>
        ...
        {/* Submit confirm */}
        {showSubmitConfirm && (
          <div className="fixed inset-0 z-50 grid place-items-center bg-ink-950/80 backdrop-blur-sm p-5">
            <div className="w-full max-w-md rounded-xl border border-ink-800 bg-ink-900 p-6">
              <h3 className="font-display text-xl text-paper mb-2">Submit your exam?</h3>
              <p className="text-sm text-ink-300 mb-5">
                You've answered <strong className="text-paper">{answeredCount}</strong> of {questions.length} questions.
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="press flex-1 rounded-md border border-ink-700 px-4 py-2.5 hover:border-ink-500 tx-color"
                >
                  Keep going
                </button>
                <button
                  onClick={() => { setShowSubmitConfirm(false); finish(); }}
                  className="press flex-1 rounded-md bg-meth text-ink-950 px-4 py-2.5 font-medium hover:bg-meth-300 tx-color"
                >
                  Yes, submit
                </button>
              </div>
