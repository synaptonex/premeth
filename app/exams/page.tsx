    ...
    <>
      ...
      <main ref={root} className="mx-auto max-w-6xl px-5 py-14">
        <div className="flex flex-col gap-3 mb-10">
          <span className="text-xs uppercase tracking-widest text-meth">Catalog</span>
          <h1 className="font-display text-5xl md:text-6xl text-paper tracking-tight">
            All 31 categories.
          </h1>
        ...
        {GROUPS.map((g) => {
          ...
          return (
            <section key={g} className="mb-12">
              <h2 className="text-xs uppercase tracking-widest text-ink-500 mb-4">
                {g}
              </h2>
                ...
                {groupItems.map((c) => {
                  ...
                  return (
                    <Link
                      key={c.slug}
                      href={`/papers/${c.slug}`}
                      className="exam-card group relative p-5 rounded-xl border border-ink-800 bg-ink-900/40 hover:border-meth/40 tx-color block"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <h3 className="font-display text-xl text-paper group-hover:text-meth tx-color">
                          {c.name}
                        </h3>
                        <ChevronRight
                          className="h-4 w-4 text-ink-500 mt-1 shrink-0 transition-transform duration-200 ease-out-strong group-hover:translate-x-1 group-hover:text-meth"
                        />
                      </div>
